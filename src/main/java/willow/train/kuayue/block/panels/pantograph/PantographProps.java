...
public class PantographProps {
    ...
    public void heightListInit(PantographProps pantographProps, double risePullRodAngle, double downPullRodAngle) {
        ...
        for (double pullRodAngle = risePullRodAngle; pullRodAngle <= downPullRodAngle; pullRodAngle += 0.10) {
    ...
    public double getPantographAngleByList(List<PantographMapping> heightList, double bowHeight) {
        ...
        if (right >= 0 && left < heightList.size()) {
            double rodAngle1 = heightList.get(left).getPullRodAngle();
            double rodAngle2 = heightList.get(right).getPullRodAngle();
            return (rodAngle1 + rodAngle2) / 2.0;
        }
